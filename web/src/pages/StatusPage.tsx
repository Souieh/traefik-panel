...
export default function StatusPage() {
      ...
      <CardContent className="p-4 space-y-3">
        <div className="space-y-2">
          <div className="flex items-center justify-between text-sm">
            ...
            <code
              className="bg-muted px-2 py-1 rounded text-xs font-mono truncate max-w-[200px]"
              title={router.rule}
            >
              {router.rule}
            </code>
        ...
        {showRawData && (
          <div className="mt-3 pt-3 border-t">
            <pre className="text-[10px] bg-muted p-2 rounded overflow-auto max-h-[150px] font-mono">
          ...
          <div className="text-sm">
            ...
            <div className="mt-1 bg-muted p-2 rounded text-xs font-mono overflow-x-auto">
              {Object.entries(middleware)
                .filter(
                  ([key]) =>
                    !["name", "status", "provider", "type", "usedBy"].includes(
                      key
                    )
                )
                .map(([key, value]) => (
  ...
  const filterItems = <
    T extends { name: string; status: string; provider: string }
  >(
    items: T[]
  ) => {
  ...
  const getStats = () => {
    const enabledRouters = data.routers?.filter(
      (r) => r.status === "enabled"
    ).length;
    const enabledServices = data.services?.filter(
      (s) => s.status === "enabled"
    ).length;
    const enabledMiddlewares = data.middlewares?.filter(
      (m) => m.status === "enabled"
    ).length;

    const internalRouters = data.routers?.filter((r) =>
      r.provider.includes("internal")
    ).length;
    const internalServices = data.services?.filter((s) =>
      s.provider.includes("internal")
    ).length;
